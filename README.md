...
# 20章 静的解析
コードを実行せずにコードベースを検査して潜在的なエラーや矛盾を探す手法  
## リンター
コードベースに含まれるプログラミングミスやスタイル違反を検査する  
PythonではPylintが有名である  
以下のような機能がある  
- PEP8
- デットコード
- アクセス制限違反
...
Pylintはプラグインを活用することでより、複雑な動作が可能となる  
## その他の静的解析ツール
### McCabe
プログラミングの循環的複雑度を測定することができる  
###  Bandit
多様なセキュリティ血管を検査することができる  

# 20章 テスト戦略
## テスト戦略の立て方
**テスト戦略**とはリスク軽減のためにソフトウェアテストにどれだけ時間と労力を費やすかに関する計画  
どこに重点を置いてテストを書くかはテスト戦略によって決まる  
テストを考える際に一番最初に実施することである  
### テストとは何か
開発者はソフトウェアを書くにあたり、**何を**書いているのか、**なぜ**書いているのかを理解しないといけない  
これを理解することで、テストを目的が明らかになる  
**なぜ**を保証するためにテストが存在する  
テストのタイプとそのテストが答えてくれる問い  
| テストタイプ | テストが答える問い |
| :--- | :--- |
| ユニット | ユニットが開発者の期待通りに動作するか |
| インテグレーション | システムの異なる部分がピッタリとフィットするか |
| 受け入れ | システムのエンドユーザーの期待通りに動作するか |
| 負荷 | 負荷が重くなってもシステムは動作し続けるか |
| セキュリティ | システムが特定の攻撃等を防げるか |
| ユーザビリティ | システムが直感的に使えるか |

## AAAテスト
Arrange-Act-Assert(準備、実行、検証)の頭文字  
### Arrange(準備)
テストに必要なデータを準備するコードの部分  
このブロックが大きいのは危険信号である  
テスト全体を眺めて、全てのテストで同じになる条件は何かを考えて、それかを関数で抽出し、各テストでその関数を使い回すほうがよい  
ほとんどのテストフレームワークにはテストの前に自動的にコードを実行してくれる方法を提供しているので、それを活用する  
pytestの**フィクスチャ**が上記に該当する  
### Annihilate(クリーンアップ)
テストの最後に実施する手順  
テストはお互いに独立していたほうがよく、テストの実行順によってテストが失敗したり成功したりするのは開発者を惑わしてしまう  
上記の問題を解決するために、コンテキストマネージャやフィクスチャを使用することによって、クリーンアップコード実行がされない問題を解決することができる  
### Act(実行)
テスト対象の実際の動作を具体化する  
同じ対象を異なるパラメータやアサーションで実行したい時は、テストのパラメータ化を検討する  
パラメータ化とは、同じテストを異なるパラメータで実行する方法であり、**テーブル駆動テスト**が実現できる  
テストをパラメータ化すると多数のテストケースを1つに圧縮することができる  
### Assert(検証)
クリーンアップを行う前に最後のステップはシステムが何らかのプロパティが条件を満たすことの検証である  
テストの終了前に論理的なアサーションが1つ(条件によっては複数)入ることが望ましい  
アサーションが複数存在するとテストが失敗する理由が複数個存在することとなり、メンテナの理解を阻害する原因になってしまう  
アサーションは、どこに問題が生じたかを開発者に中身のあるメッセージで伝えれたほうがよい  
pytestでも上記は可能であるが、**Hamcrest**を使用すればより高度で複雑なアサーションを使用することができる  